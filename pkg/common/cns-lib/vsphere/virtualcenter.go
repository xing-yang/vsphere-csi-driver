...
func (vc *VirtualCenter) connect(ctx context.Context) error {
	...
	// Assign through a local for the same reason as the initialisation branch
	// above: a failed NewClient must not leave vc.Client nil, or the next
	// connect() call would take the "client was never initialized" branch and
	// skip rebuilding the dependent clients checked below.
	var client *govmomi.Client
	if client, err = vc.NewClient(ctx, useragent); err != nil {
		log.Errorf("failed to create govmomi client with err: %v", err)
		if !vc.Config.Insecure {
			log.Errorf("failed to connect to vCenter using CA file: %q", vc.Config.CAFile)
		}
